...
2. **La confirmation se fait dans une transaction Mongo** : on vérifie qu'aucune réservation `pending`/`confirmed` ne chevauche (`startAt < newEnd AND endAt > newStart`), puis on insère — le tout atomiquement. Cela couvre les durées variables (heure, jour, semaine, mois) que l'index unique seul ne couvre pas, et le « conflit bloquant à la création » côté gestionnaire (§4.2).
...
| ------------------------ | ------------------------- | ---------------------------------------------------------------------------------------------- |
| `_id`                    | ObjectId                  |                                                                                                |
| `reference`              | String                    | n° lisible (ex. `RES-2025-00042`), unique                                                      |
...
| `expiresAt`        | Date              | = créationbnd + 10 min                     |
| `createdAt`        | Date              |                                            |

**Index** :

- `{ expiresAt: 1 }` avec `expireAfterSeconds: 0` → **purge TTL automatique**.
- `{ spaceId: 1, startAt: 1, endAt: 1 }` **unique** → mutex anti-collision (§1.2).
...
**Index** : `{ invoiceId: 1 }`, `{ "reconciliation.status": 1 }`.
...
## 10. Récapitulatif des collections (27)

**Cœur (Phase 1)** : `buildings`, `spaces`, `slotClosures`, `reservations`, `slotLocks`.
**Clients** : `clientAccounts`, `cardex`.
**Staff** : `staffProfiles`, `auditLogs`.
**Tarification** : `tariffs`, `services`, `discountCodes`.
**Facturation** : `quotes`, `invoices`, `payments`.
**Périphérique** : `notifications`, `reviews`, `satisfactionSurveys`, `newsOffers`, `incidents`, (`reservationRequests`, `statsSnapshots` optionnelles).
