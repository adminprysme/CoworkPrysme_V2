...
| **7**  | PDF devis + email send                             | template devis + lien « Accepter le devis » ; exclusion internalNote                                                                                                                                                                                                                       | #4, #2                                  |
| **8**  | Accept unifié → Reservation(s) + Invoice A1        | `AcceptQuoteService` ; paths client + staff ; `awaitingPaymentExpiresAt = validUntil` ; stub SEPA UI ; **sans** payment link finalisé (ou stub prêt)                                                                                                                                       | #2, #4, #5 ; **validation user §5.1.3** |
| **9**  | Stripe payment link + QR + email                   | `quotePaymentLinks` ; `/payer-devis` ; PI + webhook `quoteId` ; montant LOCKED #5 ; QR `qrcode`                                                                                                                                                                                            | #8                                      |
